...
wd<-getwd()
source(paste0(wd,'/Scripts/SA_functions_02122024.R'))
...
# crops <- c("corn","soy","wwheat", "hay", "alfalfa") #see about sorghum and cotton from Emily
...
# AE <- readRDS(paste0(wd,"/StartingData/us_eco_l3.rds"))
    ...
    cnty_AER <- readRDS("StartingData/cnty_AER.RDS") #area.overlay(counties, AE, projection, "cnty_AER")
...
# df_spat %>%
#   group_by(GEOID) %>%
#   summarise(RICH = mean(RICH_CDL_AG)) %>%
#   ggplot() +
#     geom_sf(aes(fill = RICH), color = NA) #just checking spatial distribution - looks right for corn
    
  
  prep.data.std.new(data = df_aer, metrics = metrics, 
                    savename=paste0(crop_panels[i],"_ntiles_02142024"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
...
################################################
### Prep data subsets of each crop for INLA ### NOt in use as of 02262024
################################################


foreach (i=1:length(crop_panels))%do%{
  
  df <- readRDS(paste0(wd,"/StartingData/",crop_panels[i],"_stdLDevalpts.RDS")) #read in the data with yield, weather, control, and landscape metrics
  
  df_minyr <- df %>% group_by(GEOID) %>% #meets a minimum number of years of data
    mutate(.,keep=(n())) %>% 
    ungroup() %>% 
    filter(., keep >= 3) %>% 
    dplyr::select(., -keep)
  
  prep.data.std.new(data = df_minyr, metrics = metrics,
                    savename=paste0(crop_panels[i],"_012023_3yrs"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
  
  
  df_minag <- df %>% #meets a minimum number of sqkm of agricultural area (2.5% for corn is ~0.5 sqkm)
    filter(., AG_SQKM >= quantile(AG_SQKM, probs = c(0.025), na.rm = T)[[1]]) 
  
  prep.data.std.new(data = df_minag, metric = metrics,
                    savename=paste0(crop_panels[i],"_012023_minag"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
  
  
  df_smallcounty <- df %>% #counties that are relatively small (for corn 25% is ~1131 sqkm)
    filter(., TOTAL_SQKM < quantile(TOTAL_SQKM, probs = c(.25), na.rm = T)[[1]] ) 
  
  prep.data.std.new(data = df_smallcounty, metric = metrics,
                    savename=paste0(crop_panels[i],"_012023_smallcounty"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
  
  df_largecounty <- df %>% #counties that are relatively large (for corn 75% is ~2417 sqkm)
    filter(., TOTAL_SQKM >= quantile(TOTAL_SQKM, probs = c(.75), na.rm = T)[[1]] )
  
  prep.data.std.new(data = df_largecounty, metric = metrics,
                    savename=paste0(crop_panels[i],"_012023_largecounty"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
  
  
  df_lowag <- df %>% #counties with relatively little area devoted to ag
    mutate(PERC_AG = AG_SQKM/TOTAL_SQKM) %>%
    filter(., PERC_AG < quantile(PERC_AG, probs = c(.25), na.rm = T)[[1]] ) 
  
  prep.data.std.new(data = df_lowag, metric = metrics,
                    savename=paste0(crop_panels[i],"_012023_lowag"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
  
  
  df_highag <- df %>% #counties with relatively high area devoted to ag
    mutate(PERC_AG = AG_SQKM/TOTAL_SQKM) %>%
    filter(., PERC_AG >= quantile(PERC_AG, probs = c(.75), na.rm = T)[[1]] ) 
  
  prep.data.std.new(data = df_highag, metric = metrics,
                    savename=paste0(crop_panels[i],"_012023_highag"), 
                    projection="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs") #102003 epsg code for USA albers equal area conic
